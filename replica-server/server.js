...
const app = express();
const PORT = 5001; // Primary server port
const REPLICA_URL = "http://localhost:5000"; // Replica server URL

app.use(express.json());
const mutex = new Mutex();

// Load data from file
let inventory = JSON.parse(fs.readFileSync("replica-data.json", "utf8"));

// Save data to file
function saveData() {
  fs.writeFileSync("replica-data.json", JSON.stringify(inventory, null, 2));
}
